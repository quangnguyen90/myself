...
class Tank_auth
{
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						...
						$this->ci->session->set_userdata(array(
								...
								'author'	=> $user->authenticate_level,
								'IsAuthorized' => true,
	...
	function logout()
	{
		...
		$this->ci->session->set_userdata(array('user_id' => '', 'username' => '', 'status' => '', 'author' =>'', 'IsAuthorized' => false));
	...
	/**
	 * Get authenticate lvel
	 *
	 * @return	string
	 */
	function get_authenticator()
	{
		return $this->ci->session->userdata('author');
	}
	...
	{
		return $this->ci->session->userdata('IsAuthorized');
	}
